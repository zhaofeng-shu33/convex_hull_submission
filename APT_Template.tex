...
For the convex hull $\mathrm{H}_N$ of these $N$ points, we study its number of facets $F_N$.
...
to guarantee that the newly generated point falls in the interpolation region $\mathrm{H}_N$.
...
dependence on $d$. As far as we know, no previous works revealed the general asymptotic expression of $\E[F_N]$ in $\R^d$.
...
Let $d$ represent the distance from the origin to the hyperplane spanned by $X_1, \dots, X_d$.
Specifically, in 2-D, $d=d_{12}$ is the distance to the line passing $X_1$ and $X_2$.
The probability $H(x)=P(d\geq x)$ represents the event that the distance $d$ is larger than $x$.
...
for the Gaussian distribution and the uniform distribution within a unit sphere.

Distributions with algebraic tails $F(x) \sim x^{-k}$ are related with Poisson random process.
It is shown that for $k>0, d=2$,
$\mathrm{H}_N$ converges to the convex hull of a Poisson random process and $\E[F_N]$
...
Studies related with distribution with exponential tails mainly focus on the Gaussian distribution.
For this special distribution, Kabluchko et al. \cite{kabluchko2020absorption} obtained explicit formula for $p_{N,d}$;
...
For truncated tail,
Affentranger \cite{affentranger1991convex} studied a sub-category called beta-typed distribution and obtained
...
and can further be generalized to compute other combinatorial property of $\mathrm{H}_N$ (like surface area and volume)
...
Gaussian random variables, then $K(x) = e^{-x^2/2}$.
In fact, $1-K(x)$ is the CDF of the Rayleigh distribution.
