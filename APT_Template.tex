...
\begin{abstract}
 ...
 we derive the complete integral formula for the expectation of its facets. The formula is
 in term of the CDF of the 1D marginal distribution.
 ...
 As a byproduct, the asymptotic result is applied to estimate the sample complexity when the probability measure of convex hull
 tends to 1.
\end{abstract}
...
In 2d, $d=d_{12}$ is the distance to the random line passing $X_1$ and $X_2$. $H(x)=P(d\geq x)$ is
...
as well as Gaussian distribution (belonging to exponential tail).
 ...
 j\section{Integral formula}\label{sec:int_f}
