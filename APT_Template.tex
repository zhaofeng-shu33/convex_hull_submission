...
We use $H(x)=P(\dist\geq x)$ to denote the probability that the distance $\dist$ is larger than $x$.
...
For standard Gaussian distribution, $X_1, X_2$ are independent
...
P(\dist(P_1, \dots, P_{d'-1})\geq x) = \int_{y}^x K^{d'-2}(x)\lambda_{d'}(x/y)|\d F(y)|
$.
