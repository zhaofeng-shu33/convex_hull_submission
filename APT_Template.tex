...
equals the limit of \textcolor{red}{this random process} .
...
both of which are expressed with $F(x)$ in the following way:
\begin{align}
...
We define the auxiliary function $\lambda_d(x)$ and $K(x)$ as follows: 
     \begin{align}
...
\begin{align}
     \E[F_N] \sim \binom{N}{d} \int_0^{+\infty} 
      (1-G(x))^{N-d} |dH(x)| \textrm{ as } N-d\to \infty
...
\begin{equation}\label{eq:F_poly_tail}
     F(x) = x^{-k} L(x), k\geq 0
\end{equation}
where $L(x)$ is a slowly varying function.\textcolor{red}{Define what is a slowly varying function}
...
When $N\to \infty$ and $d$ is fixed, we have
 ...
 \end{equation}
\section{Sample complexity in data interpolation and conclusion}\label{sec:sample_complexity}
