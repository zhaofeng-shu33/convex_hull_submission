...
\begin{abstract}
  ...
  To be specific, we model the learning task as the interpolation of a test sample with i.i.d. sampled training data,
  ...
  considered in this paper. The analytical form of the sample complexity also shows that more samples are needed if the distribution tail decays faster,
  Therefore, our results provide the theoretical justification for methods of dimension reduction and data transformation to reduce the sample complexity.
\end{abstract}
...
convex hull of training dataset.

The above background motivate us to investigate how to make interpolation ``work''.
A naive way is to increase the training sample size. Then it is natural to ask how many samples are needed.
To quantitatively determine the sample size, we follow the geometric statistical model described above, and then derive the sample size which makes
...
As a result, it is beneficial to apply some non-linear transformation to flatten the data
...
expressions of $\E[F_{N,d}]$ and the sample complexity for $p_{N,d}<\epsilon$ as $d\to\infty$ and $N$ is a function of $d$.
...
It should be emphasized that the probability considered above is different from that of the convex position,
