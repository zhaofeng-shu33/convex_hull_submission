...
\shorttitle{On the expectation of facets of the convex hull of samples} % insert short title here for use in running head
...
\title{On the expectation of facets of the convex hull of samples from spherically symmetric distributions} % insert title - use \\ if it requires more than one line.
...
\begin{abstract}
 For the convex hull of d-dimensional samples from spherically symmetric distributions,
 we derive the integral formula for the expectation of its facets.
 As the number of samples approach infinity,
 the integral formula enables us to obtain the precise asymptotic value of the expectation of facets
 for three types of spherically symmetric distributions.
 The asymptotic result is applied to estimate the sample complexity for the interpolation of random points.
\end{abstract}

\keywords{Convell hull; spherically symmetric distribution; expectation of facets}
...
Let $X_1, X_2, \dots, X_N$ be i.i.d. random points from a spherical symmetric distribution in $\mathbb{R}^d$.
The convex hull $H_N$ of these $N$ points are an uninterrupted research topic in stochastic and integral geometry
\cite{schneider2008stochastic}.
The main aim of this paper is to provide a integral formula for $\E[F_N]$, the mathematical expectation
of the number of $d-1$ dimensional facets
of $H_N$,
and derive its asymptotic expression
for three kinds of distribution families. Furthermore,
let $X$ be a d-dimensional random vector from the same distribution but independent with $X_1, \dots, X_N$.
The estimation of $\E[F_N]$ is used to obtain the upper bound
of the probability $p_{N,d}=P(X \not\in H_N)$, the probability content outside of the random convex hull.
When the training data are sampled from the Gaussian distribution,
the upper bound of this probability can be applied to
explain the dimensional curse \cite{balestriero2021learning}.
That is,
to maintain $p_{N,d} \to 0$, $N(d)$ grows faster than an exponential function of $d$.

The asymptotic expression of $\E[F_N]$ as $N\to \infty$
was studied firstly by R{\'e}nyi and Sulanke \cite{renyi1963konvexe}.
The distributions they considered
are 2D gaussian and uniform distribution in a convex region with continuous curvature.
Later on, Carnal \cite{carnal1970konvexe} generalized R{\'e}nyi's work.
He classified symmetric distribution in 2d
into three types with different tails:
polynomial, exponential and truncated,
and he obtained the asymptotic expression of $\E[F_N]$
for each type.

The study of $\E[F_N]$ for $d>2$ is made firstly by
Raynaud
...
estimated the order of $\E[F_N]$ in big Theta
notation
for three different distribution families.

The existing work has not revealed the asymptotic formula of $\E[F_N]$ in $\R^d$.
In Section \ref{sec:int_f}, we obtain this formula by generalizing Carnal's integral formula
of $H(x)=P(d_{12}\geq x)$ from $d=2$ to higher dimensions.
Then we derive the asymptotic
...
we discuss the condition for the convergence of $p_{N,d}$ to zero as $N,d \to \infty$
...
a d-sphere. $G(x)=P(X_1\geq x)$ corresponds to the one dimensional marginal distribution.
Let $d$ represents the distance from the origin to the random hyperplane, spanned by $X_1, \dots, X_d$.
In 2d, $d=d_{12}$, the distance to the random line passing $X_1$ and $X_2$. $H(x)=P(d\geq x)$ is
the probability that the distance $d(X_1, \dots, X_d)$ is larger than $x$.

\section{Related work}
We list some other related works which have
not been mentioned in the previous section.
...
It is shown that in 2d for $k>0$ the convex hull converges to a Poisson random process and $\E[F_N]$
...
and Gaussian distribution (belonging to exponential tail).
...
and can further to generalized to compute other combinatorial property of $H_N$ (like surface area and volume)
...
vertices of $H_N$.
By far it is not clear whether there exists the explicit relation between $\E[F_N]$ and $\E[V_N]$
for general spherical symmetric distribution.
However, we have some inequality estimations. Using Corollary 19.6 of \cite{brondsted2012introduction}, we have the following
inequality:
