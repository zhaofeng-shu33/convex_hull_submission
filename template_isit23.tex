...
abilities of machine learning algorithms \cite{webb2020learning}.
Under this interpretation, interpolation occurs whenever a new sample belongs to the convex hull of training samples;
and extrapolation occurs when the sample falls outside.
Many researchers agree that
without careful design, the ability of these algorithms is limited to interpolation between training data points.
However, in recent years some researchers argue against this view from geometric understanding of the data distribution.
...
When they take $N$ samples from a certain distribution,
they found the new or ($N+1$)-th sample falls in the convex hull of the first $N$ samples
with high probability. Therefore, extrapolation occurs in prediction regardless of algorithms in use.
...
we find that they are ambivalent on the actual data dimension
...
we consider three distribution families, which cover mostly common used statistical models:
polynomial,
...
Apart from augmenting the anti-interpolation view in the last paragraph,
our work also sheds insights on
the functionality of dimension reduction and non-linear transformations in machine learning pipeline.
In high dimensional spaces, the relationship between $N$ and $d$,
Besides, it is beneficial to apply some non-linear transformations to flatten the data
in the preprocessing step. we also reveal the dependency of $N$ on the decaying rate of the distribution tail.
