...
\label{sec:intro}
In the field of machine learning, intuitively we model a learning task as interpolating the
training data \cite{webb2020learning}.
Under this interpretation, we are prone to think that state-of-the-art
algorithms work well in high-dimensional spaces because the models can interpolate training data well. 
However, some researchers also argue against this view from different perspectives.
For example, Balestriero et al. \cite{balestriero2021learning}
thinks that this is a misconception and that interpolation almost surely never occurs in high-dimensional spaces.
In their terminology, interpolation occurs whenever a new sample belongs to the convex hull of previous samples.
...
The above background motivates us to investigate when interpolation occurs
with a high probability as we increase the training sample size.
To solve this problem analytically, we follow the geometric statistical model described above and then derive the sample size, which makes
interpolation almost surely occurs if the distribution is spherically symmetric.
This sample size, or sample complexity, can be used to estimate the required number of samples
to achieve high precisions of the interpolation. Differing from previous
studies which mostly consider special distributions, we obtain the results of sample complexity
in three distribution families, which cover mostly common used statistical models.
We show that when the training data are sampled from polynomial,
exponential and truncated distribution families,
we need an exponentially large number of samples for vanishing interpolation error.
...
before constructing an interpolation representation of training samples.
Besides the relationship between the sample size $N$ and the feature dimension $d$,
we also reveal the dependency of $N$ on the decaying rate of the distribution tail.
As a result, it is beneficial to apply some non-linear transformations to flatten the data
...
Moreover, the upper bound is closely related with the quantity $\E[F_{N,d}]$,
...
The geometrical meaning of  $p_{N,d}$ is explained as follows: Take $N$ samples from a certain distribution
and consider whether the new or ($N+1$)-th sample falls in the convex hull of the first $N$ samples. 
If $p_{N,d}$ is small, the new sample falls outside with high probability.

It should be noted that the probability $p_{N,d}$ considered above
is different from that of the convex position,
which requires all samples are the vertices of the convex hull \cite{Valtr1995}. The latter probability decreases much faster
than $p_{N,d}$ as $N$ increases.
...
To study $p_{N,d}$, we introduce two additional quantities of $H_N$: the number of vertices
