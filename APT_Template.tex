...
The number of facets ($(d-1)$-dimensional faces) $F_N$ is a combinatorial property of the convex hull $H_N$ of these $N$ points.
The main contribution of this paper is to provide a complete integral formula for $\E[F_N]$,
the mathematical expectation
of $F_N$ in $H_N$, and derive its asymptotic expression
for three categories of distributions.

Our motivation to study $\E[F_N]$
is using it to give an upper bound of
the probability $p_{N,d}=P(X_{N+1} \not\in H_N)$,
which is the probability content outside of the random convex hull.
This probability comes from an approach to explain
why interpolation is extremely difficult in high dimensions \cite{balestriero2021learning}.
In other words, we need at least exponentially large quantities of samples $N$
to guarantee that all newly generated point falls within $H_N$.
In our work, the asymptotic result on $\E[F_N]$ can be directly used directly to obtain
a sufficient condition on $N,d$ for $p_{N,d} \to 0$.
