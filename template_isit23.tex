...
Then we introduce three distribution tail functions $F(x),G(x),H(x)$,
...
the asymptotic expression of $\E[F_N]$ is derived
for three different distribution families.
...
\begin{theorem}\label{thm:poly_tail_sample_complexity}
  ...
  when the following condition is satisfied, the interpolation error $p_{N,d} < \epsilon$ as $d\to \infty$.
...
From the above theorem, we see that in order to guarantee the interpolation error is no more than a given precision $\epsilon$,
The sample size $N$ has an exponential relationship with the dimension $d$.
...
\begin{definition}
  ...
  $x = L(1/F(x))$ holds for the given tail distribution function $F(x)$,
  we say that the distribution has exponential tails.
\end{definition}
...
\begin{theorem}\label{thm:truncated_tails}
     ...
     \end{align}     
     Besides, we have
 the following statements about $\E[F_N]$:
\begin{itemize}
