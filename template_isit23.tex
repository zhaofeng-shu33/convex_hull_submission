...
\begin{abstract}
  ...
  To be specific, we model the learning task as interpolation of the convex hull consisting of i.i.d. sampled data,
  and the sample complexity is the number of training data which ensures that
  test samples fall into the convex hull
  with large probability. It is shown that the sample complexity has exponential relationship with the feature dimension for various distribution families
  considered in this paper. Our analytical form of the sample complexity also shows that more samples are needed if the distribution decays faster.
\end{abstract}
...
It seems that the authors in \cite{balestriero2021learning} confuse the two models.
...
before applying machine learning models. Besides the relationship between the sample size $N$ and the dimension $d$,
...
the expectation of the number of facets in the convex hull. Loosely speaking,
