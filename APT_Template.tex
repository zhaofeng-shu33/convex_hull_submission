...
with respect to the CDF of the radial distribution.
 ...
 for three types \tedit{categories} of spherically symmetric distributions.
 As a byproduct \tedit{Additionally}, the asymptotic result is \tedit {can be} applied to estimating the sample complexity when the probability measure of the convex hull
 tends to 1.
\end{abstract}
...
Let $X_1, X_2, \dots, X_N$ be i.i.d. random points generated from a spherical symmetric distribution in $\mathbb{R}^d$.
The convex hull $H_N$ of these $N$ points is defined as the smallest convex set containing them \tedit{cite}.
Various quantities related with $H_N$, including the number of vertices, the number of facets, the volume, and the surface area,
were investigated
extensively by previous works \cite{schneider2008stochastic, barany2008random, hueter1999limit}.
