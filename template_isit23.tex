...
argues that this is a misconception and that interpolation almost surely never occurs in high-dimensional spaces.
Their logic is as follows. Take $N$ samples from a certain distribution
...
In addition to their empirical study, further experiments on other real datasets \cite{yousefzadeh2022extent} also show that
...
To solve this problem quantitatively, we follow the geometric statistical model described above and then derive the sample size, which makes
...
For a slowly varying function $L(x)=\log{x}$ and exponential distribution with $F(x)=e^{-x}$, we have $L(1/F(x))=x$.
Generally, we define distribution with exponential tails as follows.
\begin{definition}
